/// Register `CIACRA` reader
pub type R = crate::R<CiacraSpec>;
/// Register `CIACRA` writer
pub type W = crate::W<CiacraSpec>;
/// Start/stop timer A. SIDE-EFFECT: writing Enabled begins the count
/// immediately.
...
/// Force-load timer A from its latch. A strobe: writing Execute performs the
...
/// Field `FORCE_LATCHED_LOAD` writer - Force-load timer A from its latch. A
...
impl R {
    /// Bit 0 - Start/stop timer A. SIDE-EFFECT: writing Enabled begins the
    /// count immediately.
    #[inline(always)]
    pub fn start_timer(&self) -> StartTimerR { StartTimerR::new((self.bits & 1) != 0) }

    /// Bit 1 - Route timer A underflow onto PB6 (Enabled) instead of using it
...
impl W {
    ...
    /// count immediately.
    #[inline(always)]
    pub fn start_timer(&mut self) -> StartTimerW<'_, CiacraSpec> { StartTimerW::new(self, 0) }

    /// Bit 1 - Route timer A underflow onto PB6 (Enabled) instead of using it
    /// as general I/O (Disabled)
    #[inline(always)]
    pub fn select_timer_output(&mut self) -> SelectTimerOutputW<'_, CiacraSpec> {
        SelectTimerOutputW::new(self, 1)
    }

    /// Bit 2 - PB6 output waveform on timer A underflow
    #[inline(always)]
    pub fn port_output_mode(&mut self) -> PortOutputModeW<'_, CiacraSpec> {
        PortOutputModeW::new(self, 2)
    }

    /// Bit 3 - Timer A reload behavior after underflow
    #[inline(always)]
    pub fn timer_run_mode(&mut self) -> TimerRunModeW<'_, CiacraSpec> {
        TimerRunModeW::new(self, 3)
    }

    /// Bit 4 - Force-load timer A from its latch. A strobe: writing Execute
    ...
    #[inline(always)]
    pub fn force_latched_load(&mut self) -> ForceLatchedLoadW<'_, CiacraSpec> {
        ForceLatchedLoadW::new(self, 4)
    }

    /// Bit 5 - What timer A counts
    #[inline(always)]
    pub fn timer_input_mode(&mut self) -> TimerInputModeW<'_, CiacraSpec> {
        TimerInputModeW::new(self, 5)
    }

    /// Bit 6 - Serial-port shift-register direction
    #[inline(always)]
    pub fn serial_port(&mut self) -> SerialPortW<'_, CiacraSpec> { SerialPortW::new(self, 6) }

    /// Bit 7 - TOD clock input frequency
    #[inline(always)]
    pub fn tod_clock_freq(&mut self) -> TodClockFreqW<'_, CiacraSpec> {
...
///
/// You can [`read`](crate::Reg::read) this register and get [`ciacra::R`](R). You can [`reset`](crate::Reg::reset), [`write`](crate::Reg::write), [`write_with_zero`](crate::Reg::write_with_zero) this register using [`ciacra::W`](W). You can also [`modify`](crate::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api).
pub struct CiacraSpec;
impl crate::RegisterSpec for CiacraSpec {
    type Ux = u8;
}
/// `read()` method returns [`ciacra::R`](R) reader structure
impl crate::Readable for CiacraSpec {}
/// `write(|w| ..)` method takes [`ciacra::W`](W) writer structure
impl crate::Writable for CiacraSpec {
    type Safety = crate::Unsafe;
}
/// `reset()` method sets CIACRA to value 0
impl crate::Resettable for CiacraSpec {}
